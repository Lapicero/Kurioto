from __future__ import annotations

from kurioto.agent import KuriotoAgent
from kurioto.config import ChildProfile
from kurioto.safety.base import (
    SafetyAction,
    SafetyCategory,
    SafetyResult,
    SafetySeverity,
)

child = ChildProfile(
    child_id="child_redirect",
    name="Alex",
    age=8,
    age_group=ChildProfile.get_age_group(8),
    interests=["space"],
)


def _make_agent() -> KuriotoAgent:
    return KuriotoAgent(child)


async def test_parent_alert_logged_on_redirect(monkeypatch):
    """Test that parent alerts are logged when content is redirected."""
    agent = _make_agent()

    # Mock SafetyAgent to return REDIRECT action
    async def fake_pre_check(user_input: str):
        return SafetyResult(
            action=SafetyAction.REDIRECT,
            reason="age-inappropriate topic, redirecting to safer content",
            severity=SafetySeverity.MEDIUM,
            categories=[SafetyCategory.AGE_INAPPROPRIATE],
        )

    class FakeAlert:
        subject = "Content Redirect - Attention Recommended"
        message = "Content was redirected to age-appropriate alternatives."
        follow_up_recommended = True
        urgency = "medium"

    async def fake_generate_parent_alert(user_input: str, sr: SafetyResult):
        return FakeAlert()

    monkeypatch.setattr(agent.safety_agent, "pre_check", fake_pre_check)
    monkeypatch.setattr(
        agent.safety_agent, "generate_parent_alert", fake_generate_parent_alert
    )

    # Process a message that triggers REDIRECT
    _ = await agent.process_message("tell me about adult topics")

    # Retrieve parent dashboard logs
    dashboard = agent.tools["parent_dashboard"]
    logs = await dashboard.execute(action="get_logs")
    recent = logs.data["recent_logs"]

    # Assert that a parent_alert entry was logged for redirect
    parent_alerts = [e for e in recent if e["type"] == "parent_alert"]
    assert len(parent_alerts) >= 1
    alert_entry = parent_alerts[-1]
    assert "Redirect" in alert_entry["data"]["subject"]
    assert alert_entry["data"]["urgency"] == "medium"
    assert alert_entry["data"]["follow_up_recommended"] is True
